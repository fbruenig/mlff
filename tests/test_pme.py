...
def test_random_structure(
    sr_cutoff, frame_index, scaling_factor, ortho, calc_name
):
    """
    ...
    if calc_name == "ewald":
        ...
        atoms.info.update(k_grid = get_kgrid_ewald(jnp.array(atoms.get_cell()), k_spacing))
    ...
    volume = jnp.abs(jnp.dot(jnp.cross(atoms.cell[0], atoms.cell[1]), atoms.cell[2]))
